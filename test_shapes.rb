...
class TestSegment < Test::Unit::TestCase
  ...
  def test_location_has_x_and_y
    assert_equal (100.0 / 3.0).round(8), @location.x.round(8)
  ...
  def test_get_p2_from_segment
    ...
    assert_equal lc_p2.x.round(8), @segment.p2.x.round(8)
    assert_equal lc_p2.y.round(8), @segment.p2.y.round(8)
  end
  ...
  def test_get_radian_from_segment_made_from_p1_p2
    ...
    assert_equal (Math::PI * 1.0 / 3.0).round(8), segment.radian.round(8)
    assert_equal 1, segment.length.round(8)
  end

  def test_radian_is_always_positive_1
    @segment.radian -= Math::PI
    assert_equal (Math::PI * 4.0 / 3.0).round(8), @segment.radian.round(8)
  end
  ...
  def test_p2_changes_when_radian_is_changed
    ...
    assert_equal lc_p2.x, @segment.p2.x.round(8)
    assert_equal lc_p2.y, @segment.p2.y.round(8)
  end
  ...
  def test_rotate
    ...
    assert_equal new_radian.round(8), segment_after_rotate.radian.round(8)
  end
...
class TestLocation < Test::Unit::TestCase
  ...
  def test_location_distance_method
    ...
    assert_equal (5**(1.0 / 2.0)).round(8), loc.distance(loc1).round(8)
  end
